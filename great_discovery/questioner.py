...
def generate_questions(conn, epoch, profiles):
    ...
    for profile in profiles:
        ...
        # Log to holes table
        c.execute(
            "INSERT INTO holes (epoch_found, shape_sig, demands, filled, filled_by, src_id, dst_id) VALUES (?,?,?,0,NULL,?,?)",
            (epoch, f"{profile['src_id']}-{profile['dst_id']}", q['question'], profile['src_id'], profile['dst_id'])
        )
...
def generate_meta_questions(conn, epoch, meta_holes):
    ...
    for (src_id, dst_id) in meta_holes[:2]:   # max 2 per epoch
        ...
        c.execute(
            "INSERT INTO holes (epoch_found, shape_sig, demands, filled, filled_by, src_id, dst_id) VALUES (?,?,?,0,NULL,?,?)",
            (epoch, f"meta:{src_id}-{dst_id}", mq['question'], src_id, dst_id)
        )
