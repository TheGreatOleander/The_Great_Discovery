...
def init_db(path="discovery.db"):
    ...
    c.execute("""
        CREATE TABLE IF NOT EXISTS holes (
            ...
            filled_epoch INTEGER DEFAULT NULL
        )
    """)

    # Migration: add columns to existing databases created before this version
    for col, typedef in [
        ("src_id",       "INTEGER DEFAULT NULL"),
        ("dst_id",       "INTEGER DEFAULT NULL"),
        ("filled_epoch", "INTEGER DEFAULT NULL"),
        ...
        try:
            c.execute(f"ALTER TABLE holes ADD COLUMN {col} {typedef}")
        except Exception:
