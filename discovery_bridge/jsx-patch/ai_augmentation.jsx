...
// AI-Augmented Hole Settling — BRIDGE EDITION
//
// CHANGES FROM ORIGINAL:
//   - askClaude() removed entirely
//   - askStructuralBridge() replaces it (routes through n8n, no exposed key)
//   - buildStructuralPayload() constructs canonical event format
//   - validateBridgeResponse() gates all injection — nothing enters graph unchecked
//   - conceptAlreadyExists() prevents duplicate node injection
//   - WEBHOOK_URL read from config.js (single file to edit for deployment)
//
// WHAT DID NOT CHANGE:
//   - All engine math (buildField, softmax, measureCompression, findHoles, etc.)
//   - All graph physics
//   - All rendering
//   - All UI state
//   - The structural question composition (composeQuestion)
//   - The node injection shape
//
// The JSX is now a sensor + renderer. It does not call any AI directly.
// ═══════════════════════════════════════════════════════════════════════════

// ── Bridge configuration ──────────────────────────────────────────────────────
// Edit config.js to change deployment target. Do not hardcode here.
...
const TIMEOUT_MS = (typeof window !== "undefined" && window.BRIDGE_CONFIG)
  ...
  : 30000;
const MIN_CONFIDENCE = (typeof window !== "undefined" && window.BRIDGE_CONFIG)
  ? window.BRIDGE_CONFIG.MIN_CONFIDENCE
  : 0.4;
const ALLOWED_RELATION_TYPES = (typeof window !== "undefined" && window.BRIDGE_CONFIG)
  ? window.BRIDGE_CONFIG.ALLOWED_RELATION_TYPES
  : ["supports", "contradicts", "extends", "bridges"];
...
// ─── Bridge: structural payload builder ───────────────────────────────────────
...
function buildStructuralPayload(question, profile) {
  return {
    ...
    question: question,
    allowed_domains: Object.keys(CONCEPTS),
    allowed_concepts: CONCEPTS,
  };
}

// ─── Bridge: response validator ───────────────────────────────────────────────
// Hard gate. Throws on any violation. Nothing enters graph unchecked.
...
function validateBridgeResponse(data) {
  ...
  if (data.confidence < MIN_CONFIDENCE) {
    throw new Error(`Confidence ${data.confidence} below threshold ${MIN_CONFIDENCE}`);
  }
  if (typeof data.reasoning !== "string") {
    data.reasoning = "No reasoning provided.";
  }
  if (data.relations !== undefined && !Array.isArray(data.relations)) {
    throw new Error("Relations field must be an array");
  }
...
// ─── Bridge: deduplication check ─────────────────────────────────────────────

function conceptAlreadyExists(nodes, concept, domain) {
  return nodes.some(n => n.concept === concept && n.domain === domain && !n.isRecursion);
}

// ─── Bridge: main call (replaces askClaude) ───────────────────────────────────
...
async function askStructuralBridge(question, profile) {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      throw new Error(`Bridge timed out after ${TIMEOUT_MS}ms`);
    }
...
export default function GreatDiscovery() {
  ...
  // ─── AI queue processor ───────────────────────────────────────────────────
  // Uses askStructuralBridge instead of askClaude.
  // Adds deduplication check before injection.
  ...
  const processAIQueue = useCallback(async () => {
    ...
    try {
      // ── BRIDGE CALL (replaces direct Anthropic fetch) ──
      const ai = await askStructuralBridge(question, profile);
      if(!ai) return;
      ...
      // ── DEDUPLICATION CHECK (was missing in original) ──
      if(conceptAlreadyExists(S.nodes, ai.concept, ai.domain)) {
        console.warn(`Bridge returned duplicate concept '${ai.concept}' (${ai.domain}) — skipping injection`);
        setUi(prev => ({
          ...prev,
          events: [{type:'ai',text:`↯ Duplicate concept '${ai.concept}' skipped`,epoch:S.epoch,id:Date.now()}, ...prev.events.slice(0,19)]
        }));
        return;
      }
      ...
      setUi(prev => ({
        ...
        events: [{type:'ai',text:`↯ Bridge → '${ai.concept}' (${ai.domain}) ${((ai.confidence||0)*100).toFixed(0)}% conf`,epoch:S.epoch,id:Date.now()}, ...prev.events.slice(0,19)]
      }));
      ...
      setUi(prev => ({
        ...prev,
        events: [{type:'error',text:`⚠ Bridge: ${e.message}`,epoch:E.current.epoch,id:Date.now()}, ...prev.events.slice(0,19)]
      }));
    } finally {
  ...
  const runEpoch = useCallback(() => {
    ...
    if(S.epoch>=8 && S.epoch%3===0) {
      ...
      for(const h of holes.slice(0,2)) {
        ...
        events.push({type:'question',text:`◉ Q#${S.qCount} [${q.type.toUpperCase()}] queued`,epoch:S.epoch,id:Date.now()+1});
      }
    }
    ...
    <div style={{background:'#07070c',color:'#d4d0c8',fontFamily:'"Courier New",Courier,monospace',fontSize:'11px',height:'100vh',display:'flex',flexDirection:'column',overflow:'hidden',userSelect:'none'}}>
      ...
      <header style={{borderBottom:'1px solid #141424',padding:'7px 16px',display:'flex',alignItems:'center',gap:14,flexShrink:0,background:'#08080e',position:'relative'}}>
        ...
        <div>
          ...
          <div style={{fontSize:'8px',color:'#363650',letterSpacing:'.18em',textTransform:'uppercase',marginTop:1}}>Phase 4.5 — Bridge Edition</div>
        </div>
      ...
      <div style={{flex:1,display:'grid',gridTemplateColumns:'1fr 196px 244px 230px',gap:1,background:'#0e0e1a',overflow:'hidden',minHeight:0}}>
        ...
        <div style={{background:'#07070c',padding:'8px 10px',display:'flex',flexDirection:'column',gap:7,overflow:'hidden'}}>
          ...
          <div><Row label="Entropy" val={entropy.toFixed(4)} valCol="#c8a96e"/></div>
          <div><Row label="Δ Compression" val={`${delta>=0?'+':''}${delta.toFixed(4)}`} valCol={delta>0.05?'#e05c5c':delta<-0.02?'#44ff99':'#545268'}/></div>
            ...
            {questions.map(q=>{
              ...
              return (
                <div key={q.id} style={{border:`1px solid #0e0e1a`,borderLeft:`3px solid ${c}`,padding:'7px 9px',background:`rgba(${c==='#88ddff'?'136,221,255':c==='#aa77ff'?'170,119,255':'255,68,68'},.04)`,position:'relative'}}>
                  ...
                  <div style={{fontSize:'8px',color:'#363650',marginTop:4}}>precision {((q.precision||0)*100).toFixed(0)}% · {answered?'bridge responded':'awaiting bridge ↯'}</div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{background:'#07070c',display:'flex',flexDirection:'column',overflow:'hidden'}}>
          ...
          <div style={{flex:1,overflowY:'auto',padding:'5px 7px',display:'flex',flexDirection:'column',gap:5}}>
            ...
            {thinking&&aiResponses.length===0&&<div style={{color:'#dd88ff',fontSize:'10px',fontStyle:'italic',padding:'6px 0',opacity:.7}}>Bridge processing...</div>}
            {aiResponses.map(r=>(
          ...
          <div style={{borderTop:'1px solid #0e0e1a',padding:'4px 8px',maxHeight:108,overflowY:'auto',flexShrink:0}}>
            ...
            {events.slice(0,12).map(ev=>(
              <div key={ev.id} style={{fontSize:'9px',borderBottom:'1px solid #0a0a14',padding:'2px 0',display:'flex',gap:6}}>
                ...
                <span style={{color:ev.type==='forbidden'?'#ff4444':ev.type==='ai'?'#dd88ff':ev.type==='question'?'#88ddff':ev.type==='error'?'#ff9944':'#545268'}}>{ev.text}</span>
              </div>
            ))}
          </div>
        </div>
